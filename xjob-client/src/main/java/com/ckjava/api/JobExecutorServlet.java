...
public class JobExecutorServlet extends HttpServlet {
	...
	private void handleJob(HttpServletRequest request,
		...
		if (jobRequest == null) {
			Map<String, String> errorMap = CollectionUtils.asHashMap(
					new String[]{"errorMsg"},
					new String[]{"Http request needs [JobRequest] param."});
			HttpResponseUtils.writeJsonResponse(response, errorMap);
			return;
		}
		...
		switch (flag) {
			case TEST: {
				...
				HttpResponseUtils.writeJsonResponse(response, jobTestResponse);

				break;
			}
			case INVOKE: {
				JobInvokeResponse invokeResp = processInvokeRequest(jobRequest);
				HttpResponseUtils.writeJsonResponse(response, invokeResp);

				break;
			}
			case EXECUTING: {
				JobExecutingResponse execResp = processExecRequest(jobRequest);
				HttpResponseUtils.writeJsonResponse(response, execResp);

				break;
			}
			case STOP: {
				JobStopResponse stopResp = processStopRequest(jobRequest);
				HttpResponseUtils.writeJsonResponse(response, stopResp);
	...
	private JobRequest getJobRequestFromHttpRequest(HttpServletRequest request)
		...
		JobRequest obj;
		try {
			obj = JsonUtils.readJavaObject(requestConent, JobRequest.class).orElse(null);
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		}
